...
vec vecAddRet(vec a,vec b,float alpha)
{
    vecAdd(a,b,alpha);
    return a;
}
...
float wrap(float a,float b)
{
    float ret;
    if(a<0)
        ret=a+float(int(-a/b)+1)*b;
    else if(a>b)
        ret=int(a)%int(b-1)-float(int(a))+a;
    else ret=a;
    //printf("wrapping %.3f %.3f -> %.3f\n",a,b,ret);
    return ret;
}
void vecWrap(vec &a)
{
    ...
    for(int i=0;i<n;i++)
    {
        for(int j=1;j<n;j++)
        {
            if(j != i)
            {
                for(int k=0;k<27;k++)
                {
                    vec r2=vecAddRet(pos[j],mv[k],1);
                    vec dr=vecAddRet(pos[i],r2,-1.0);
                    float d_ij=len(dr);
                    float f_ij=48*(pow(d_ij,-14)-0.5*(pow(d_ij,-8)));
                    vecAdd(acc[i],dr,f_ij);
                }
            }
        }
    }
