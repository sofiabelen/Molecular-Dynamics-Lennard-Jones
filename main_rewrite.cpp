...
class System {
 public:
     ...
     System(const int &n_part, const int &dim, const double &density,
...
// Second Step Leapfrog: v(t + dt) = v(t + 0.5*dt) + (0.5*dt)a(t + dt)
     ...
     void calcultions() {
         ...
         for (int i = 0; i < n_part; i++) {
             ...
             v_sum += sqrt(vel[i].len_sq());
         }
        // Temperature = (n * dim)^(-1) * v_sq_sum;
        temperature = (1.0 / (dim * n_part)) * v_sq_sum;
...
int main(int argc, char** argv) {
    ...
    for (int k = 0; k < param.n_sim; k++) {
        printf("Simulation #%d\n", k);
        System sys(param.n_part, param.dim, param.density,
                param.vel_range);
